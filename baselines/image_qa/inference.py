...
from dataset import process_raw_inputs, collate
...
def get_answers(
    ...
    if bridge_entities is not None:
        ...
        for bridge_entity in bridge_entities:
            clean_bridge_entities.add(get_closest(bridge_entity, raw_data['img_ids']))
    ...
    if not use_distractors:
        ...
        if has_answers:
            ...
            if question_type in QUESTIONS_WITH_VOCAB_ANSWERS:
                assert len(bad_indexes) == len(raw_data['img_ids']) - 1
        elif question_type in QUESTIONS_WITH_VOCAB_ANSWERS:
            bad_indexes = [i for i, img_id in enumerate(raw_data['img_ids']) if img_id not in raw_data['question']]
            if len(bad_indexes) != len(raw_data['img_ids']) - 1:
        ...
        bad_indexes = [idx for idx, img_id in enumerate(raw_data['img_ids']) if img_id not in bridge_entities]

    num_samples = len(raw_data['img_ids'])
    ...
    if has_answers:
        if question_type in QUESTIONS_WITH_VOCAB_ANSWERS:
            ...
            ground_truth_answers = [
                img_id for (img_id, answer) in zip(raw_data['img_ids'], raw_data['answers'])
    ...
    if len(inputs) == 0:
        ...
        if question_type in QUESTIONS_WITH_VOCAB_ANSWERS:
            ...
            #logits[:, answer2idx[DISTRACTOR_TOK]] -= 1.e9
            #logits[:, answer2idx[IS_ANSWER_TOK]] -= 1.e9
            #logits[:, answer2idx[IS_NOT_ANSWER_TOK]] -= 1.e9
            #best_guess = np.argmax(np.max(logits, axis=0))
            ...
            predicted_answers = [inputs[best_guess_idx]['img_id']]
        else:
            ...
            for i in range(len(logits)):
                if positive_occurrances[i]:
                    predicted_answers.append(inputs[i]['img_id'])
...
class InferenceModel():
    def __init__(self, checkpoint_path, vocab_filename, device, mode, precomputed_features_info_filename=None, use_distractors=True):
        ...
        if precomputed_features_info_filename is None:
            self.feature_extractor = FeatureExtractor()
        else:
            self.feature_extractor = None
            img_ft_filenames = pickle.load(open(precomputed_features_info_filename, 'rb'))
            image_features_dir = os.path.join(os.path.dirname(precomputed_features_info_filename), "img_features")
            self.image_id_to_features = {
                img_id: pickle.load(open(os.path.join(image_features_dir, os.path.basename(filename)), 'rb'))
                for img_id, filename in img_ft_filenames.items()}
    ...
    def predict(self, question, img_ids_or_paths, question_type, bridge_entities=None):
        ...
        for img_id_or_path in img_ids_or_paths:
            if self.feature_extractor is not None:
                ...
                if img_id not in self.image_id_to_features:
                    print("Skip image: %s since no precomputed feature is found." % img_id)
                    continue
                img_features = self.image_id_to_features[img_id]
...
if __name__ == '__main__':
    ...
    vocab_filename = os.path.join(basedir, '..', '..', 'data', 'cache', 'vocab.pickle')
    device = 'cuda:1'
    question = "What length of hair does My Church have, short or long?"
    question_type = "ImageQ"
    img_ids_or_paths = ['My Church']

    precomputed_features_info_filename = os.path.join(basedir, '..', '..', 'data', 'dev_img_info.pickle')
    inference_model = InferenceModel(checkpoint_filename, vocab_filename, device, mode, precomputed_features_info_filename)
